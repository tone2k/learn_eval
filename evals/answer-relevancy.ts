...
export const checkAnswerRelevancy = async (opts: {
  ...
  if (statementsResult.statements.length === 0) {
    return {
      ...
      metadata: {
        statements: [],
        verdicts: [],
        rationale: "Empty submission received",
      },
    };
  }

  // Step 2: Evaluate each statement for relevance
  const { object: evaluationResult } = await generateObject({
    ...
    schema: z.object({
      verdicts: z.array(z.object({
        verdict: z.enum(["yes", "no", "unsure"]).describe("The relevance verdict"),
  ...
  const scores: number[] = evaluationResult.verdicts.map(v => {
    switch (v.verdict) {
      case "yes": return 1;
      case "no": return 0;
  ...
  return {
    ...
    metadata: {
      ...
      rationale: `Evaluated ${statementsResult.statements.length} statements with average relevance score of ${averageScore.toFixed(2)}`,
    },
  };
};
